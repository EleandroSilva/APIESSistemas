...
{*******************************************************}
unit Model.DAO.Imp.Pessoa.Fisica;
...
begin
  ...
  try
    try
      FQuery
        .SQL(LSQL)
          .Params('id'      , FPessoaFisica.Id)
          .Params('idpessoa', FPessoaFisica.IdPessoa)
          .Params('cpf'     , FPessoaFisica.CPF)
          .Params('rg'      , FPessoaFisica.RG)
        .ExecSQL;
    except
      FConexao.Rollback;
...
const
  LSQL=('delete from pessoafisica where id=:id ');
begin
  Result := self;
  FConexao.StartTransaction;
 try
